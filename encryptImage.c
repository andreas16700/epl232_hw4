...
byte shuffleNMostSignificantBits(byte b, int n){
    if (n<2 || n>8)
        return b;
...
void extractAndDecryptImageData(FILE* srcImage, FILE* extractedImage, const char* password){
    ...
    while (signedByte!=EOF){
        ...
        byte decrypted = shuffleNMostSignificantBits(extractedByte,BITS_TO_USE);
...
void encryptAndHideImage(const char* shellImageName, const char* hiddenImageName, const char* password){
    ...
    while (signedByteShell!=EOF){
        ...
        byte hiddenImageByteShuffled = shuffleNMostSignificantBits((byte)signedByteHidden,BITS_TO_USE);
...
void testShuffleBits(){
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        byte shuffled = shuffleNMostSignificantBits(random,i);
